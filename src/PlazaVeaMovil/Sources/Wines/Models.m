...
@implementation StrainRecipeCollection
...
{
    [_recipeId release];
    [_recipeDelegate release];
    [super dealloc];
}

#pragma mark -
#pragma mark StrainRecipeCollection (public)

@synthesize recipeId = _recipeId, recipeDelegate = _recipeDelegate;
...
- (id)initWithRecipeId:(NSString *)recipeId delegate:(id)delegate
{
    ...
    if ((self = [super init]) != nil){
        ...
        [self setRecipeDelegate:delegate];
    }
    return self;
}
...
- (void)load:(TTURLRequestCachePolicy)cachePolicy more:(BOOL)more
{
    if (![self isLoading]) {
        TTURLRequest *request = [TTURLRequest
                requestWithURL:URL(kURLRecipeStrainCollectionEndPoint,
                    _recipeId) delegate:self];
...
- (void)requestDidFinishLoad:(TTURLRequest *)request
{
    NSDictionary *rootObject =
            [(TTURLJSONResponse *)[request response] rootObject];
    StrainRecipeCollection *collection =
            [StrainRecipeCollection strainCollectionFromDictionary:rootObject];
    ...
    [self copyPropertiesFromStrainCollection:collection];
    //TODO: llamar al delegado para incrementar el contador y llamar al super
}
@end
